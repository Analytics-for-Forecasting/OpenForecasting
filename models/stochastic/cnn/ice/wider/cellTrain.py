...
from models.stochastic.cnn.ice.wider.basic import close_ho, cell_ho, cal_eNorm, fcLayer

from task.util import set_logger, plot_all_epoch
...
class CellTrainer(Opt):
    def __init__(self, _cell, dataPack, info):
        ...
        self.host_dir  = os.path.join(dataPack.series_dir, 'trainer.{}'.format(self.training_name), 'cv{}'.format(self.cid))
    ...
    def set_optim(self, all = False):
        ...
        self.epoch_scheduler = torch.optim.lr_scheduler.StepLR(
            self.optimizer, self.step_lr, gamma=0.9)
    ...
    def training(self,):
        ...
        if os.path.exists(self.model_state_path):
            ...
            try:
                ...
                plot_all_epoch(np.array(self.loss_list), 'cTraining_loss.Wide{}'.format(self.wNum), location=self.host_dir)
            except:
    ...
    def fine_train(self, ho_active = True, report = False):
        '''
        ...
        for epoch in trange(self.max_epoch):
            ...
            # batch training
                ...
                tra_loss = tra_loss / train_len
                self.record_arch(tra_loss, epoch)
            else:
            # # whole training
                try:
                    self.optimizer.zero_grad()
                    pred = self._cell(catX)
                    _loss = mse_loss(pred, self.dataPack.catE)
                    
                    tra_loss = _loss.item()
                    
                    if '{:.4e}'.format(tra_loss) == 'nan':
                        break
                    self.record_arch(tra_loss, epoch, tag)
            ...
            if report:
                self.logger.info('{} training epoch: {} \t loss: {:.4e}'.format(tag,epoch, tra_loss))
            self.loss_list.append(tra_loss)
            self.epoch_scheduler.step()
    ...
    def record_arch(self, tra_loss, epoch, tag):
        if tra_loss <= self.best_loss:
            ...
            if self.best_epoch != 0:
                ...
                self.logger.info('{} training epoch: {} \t loss: {:.8e}'.format(tag,epoch, tra_loss))
